...
<TABLE cellSpacing=0 cellPadding=0 width=749 border=0>
  ...
  <TR vAlign=top>
    ...
    <%Tcustomer customer=dooperator.isCustomerExist(mycostomer);
    if(customer!=null){
     %>
      <p>证件类型：<%=customer.getIdType()%><input type="hidden" name="idType" value="<%=customer.getIdType()%>" ></p>
      <p>证件号码：<%=customer.getIdNumber() %><input type="hidden" name="idNumber" value="<%=customer.getIdNumber() %>" ></p>
      <p>姓名：<input type="text" name="customerName" size="20" value="<%=customer.getCustomerName() %>"></p>
      <p>性别：<input type="radio" value="M" <%if(customer.getCustomerSex().equals("M")){ %>checked<%} %> name="customerSex">男&nbsp;&nbsp; <input type="radio" <%if(customer.getCustomerSex().equals("F")){ %>checked<%} %> name="customerSex" value="F">女</p>
      <p>生日：<input type="text" name="customerBirtyday" size="20" value="<%=customer.getCustomerBirtyday() %>"></p>
      <p>联系地址：<input type="text" name="customerAddress" size="20" value="<%=customer.getCustomerAddress() %>"></p>
      <%}else{ %>
      <p>证件类型：<%=mycostomer.getIdType() %><input type="hidden" name="idType" value="<%=mycostomer.getIdType()%>" ></p>
      <p>证件号码：<%=mycostomer.getIdNumber() %><input type="hidden" name="idNumber" value="<%=mycostomer.getIdNumber() %>"></p>
      <p>姓名：<input type="text" name="customerName" size="20" value=""></p>
      <p>性别：<input type="radio" value="M" checked name="customerSex">男&nbsp;&nbsp; <input type="radio" name="customerSex" value="F">女</p>
      <p>生日：<input type="text" name="customerBirtyday" size="20" value=""></p>
      <p>联系地址：<input type="text" name="customerAddress" size="20" value=""></p>
      <%} %>
      <p>　</p>
